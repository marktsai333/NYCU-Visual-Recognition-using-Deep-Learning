...
This project implements an instance segmentation pipeline for identifying and delineating four types of cells (class1–class4) in colored medical microscopy images. The model is based on **Mask R-CNN** with a **ResNet-101 + FPN** backbone, trained end-to-end with custom small-scale anchors suited to cell-size objects (10–150 px). Inference uses **Test-Time Augmentation** (8 views: 4 flips × 2 scales) merged with per-class NMS for improved AP50.
...
The output `test-results.json` can be zipped and submitted directly to CodaBench.
...
<!-- TODO: paste leaderboard screenshot here -->